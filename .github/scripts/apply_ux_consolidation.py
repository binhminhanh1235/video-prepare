            ...
            egui::TopBottomPanel::bottom("active-work").show(ctx, |ui| {
                ...
                ui.horizontal_wrapped(|ui| {
                    ...
                    if let Some(worker) = &self.run_worker {
                        ...
                        ui.label(format!("{} retry · {}", worker.target.label(), worker.project_id));
                        ...
                        ui.label(format!("Importing {} / {}", worker.scene_id, worker.visual_id));
                    }
                });
                ui.add_space(4.0);
            });
...
repl(
    ...
    '''                    {
                ...
                if !self.project_status.is_empty() {''',
    "close new project conditional",
)

repl('if ui.button("Open project").clicked() {', 'if ui.button("Continue").clicked() {', "continue CTA")
repl(
...
repl(old_actions, new_actions, "smart project action")
...
''',
    "remove global manual path input",
)
...
repl(
    ...
    '''                self.create_script_path.clear();
                ...
                self.select_project(project);''',
    "close create panel on success",
)

helper_anchor = "fn pick_data_root_folder(current: &str) -> Result<Option<PathBuf>, String> {"
helper = r'''fn pick_visual_asset_file() -> Result<Option<PathBuf>, String> {
    #[cfg(target_os = "macos")]
    {
...
"#;
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        match Command::new("kdialog")
            .args(["--getopenfilename", ".", "--title", "Choose local image or video"])
            .output()
        {
            Ok(output) if output.status.success() => return selected_folder_from_stdout(&output.stdout),
            Ok(output) if output.status.code() == Some(1) => return Ok(None),
            ...
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
                return Err("no supported system file chooser found (tried zenity and kdialog)".to_owned());
            }
...
'''
repl(helper_anchor, helper + helper_anchor, "file picker and slug helpers")

text = text.replace('"Go to Dashboard"', '"Go to Project"')
text = text.replace('"Choose a scene from the Dashboard first."', '"Choose a scene from the Project workspace first."')
